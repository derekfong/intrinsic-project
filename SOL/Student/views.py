...
def index(request, department, class_number, year, semester):
	...
	user = request.user
	student_access = ClassList.objects.filter(cid=class_id, uid=user.id).count()
	...
	return render_to_response('student/index.html', {'class': c , 'student_access': student_access, 'instruct_access': instruct_access, 'ta_access': ta_access, },
